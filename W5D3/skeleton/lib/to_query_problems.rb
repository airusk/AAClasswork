...
def toys_that_brendon_owns_sub
  ...
  SELECT toys.name
  FROM toys
  WHERE toys.id in (SELECT toys.id
  FROM toys
  JOIN cattoys
  ON toys.id = cattoys.toy_id
  JOIN cats
...
def price_like_shiny_mouse
  ...
  #-------------------------------------------------------------
  execute(<<-SQL) 
  SELECT toy_prices.name, toy_prices.price
  FROM toys AS toy_prices
  JOIN toys AS shiny_mouse_prices
  ON toy_prices.price = shiny_mouse_prices.price
  WHERE shiny_mouse_prices.name = 'Shiny Mouse'
  AND toy_prices.name != 'Shiny Mouse'
  ORDER BY toy_prices.name ASC;
  SQL
end
...
def price_like_shiny_mouse_sub
  ...
  FROM toys
  WHERE toys.price in (SELECT toys.price
  FROM toys
...
def just_like_orange
  ...
  # DO NOT USE A SUBQUERY
  execute(<<-SQL)
  SELECT other_cats.name, other_cats.breed
  FROM cats AS orange
  JOIN cats AS other_cats ON orange.breed = other_cats.breed
  WHERE orange.name = 'Orange'
  AND other_cats.name != 'Orange'
  ORDER BY
  other_cats.name ASC;
  SQL
end
...
def just_like_orange_sub
  ...
  FROM cats
  WHERE cats.breed IN (SELECT cats.breed FROM cats WHERE cats.name = 'Orange')
...
def toys_that_jet_owns
  # Find all of the toys that Jet owns. 
  
  # Then list the the names of all
  ...
  SELECT DISTINCT cats2.name, toys.name
  FROM cats AS cats1
  JOIN cattoys AS cattoys1 ON cats1.id = cattoys1.cat_id
  JOIN toys  ON cattoys1.toy_id = toys.id
  JOIN cattoys AS cattoys2 ON toys.id = cattoys2.toy_id
  JOIN cats AS cats2 ON cattoys2.cat_id = cats2.id
  WHERE cats1.name = 'Jet'
  AND cats2.name != 'Jet'
  ORDER BY cats2.name ASC;
  SQL
end
...
def toys_that_jet_owns_sub
  ...
  # Then list the the names of all the other cats that 
  # own those toys as well as the toys names.
